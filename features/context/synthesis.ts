...
const SERVICE_RE = /freela|freelance|cliente|proposta|vender|venda|landing\s*page|\blp\b|servi[cç]o|contrato/i;
const PRODUCT_RE = /micro[-\s]?saas|saas|mvp|produto|app|aplicativo|software|lan[cç]ar|startup/i;
const MONEY_RE = /dinheiro|grana|receita|fatur|ganhar|r\$|pago|moneti/i;
const LEARNING_RE = /aprend|estud|dominar|habilidade|curso|programa[cç][aã]o|ia|intelig[eê]ncia artificial/i;
...
export function classifyGoalIntent(input: string, level?: SkillLevel): GoalIntent {
  ...
  if (SERVICE_RE.test(text) && MONEY_RE.test(text)) return "sell_service";
  if (SERVICE_RE.test(text)) return "get_clients";
  if (PRODUCT_RE.test(text) && MONEY_RE.test(text)) return "financial_goal";
...
export function synthesizeMission(input: string, options: { level?: SkillLevel; maxTitle?: number } = {}): { title: string; description: string; nextAction: string; result: string; intent: GoalIntent } {
  ...
  const nextAction = (() => {
    if (intent === "sell_service" || intent === "get_clients") return "Escolher uma Landing Page ou projeto do portfólio e definir público, problema e preço inicial.";
...
export function makeActionTask(input: string, category: Category, level?: SkillLevel): Pick<Task, "title" | "description" | "category" | "priority" | "estimatedMinutes" | "xp" | "recurring" | "completed"> {
  ...
  const title = synthesized.intent === "sell_service" || synthesized.intent === "get_clients"
    ? "Enviar uma proposta de Landing Page"
    : synthesized.nextAction;
  return {
    title: sanitizeText(title, 90),
    description: `${synthesized.nextAction} Resultado: ${synthesized.result}`,
    category,
    priority: "alta",
    estimatedMinutes: 30,
    xp: 50,
