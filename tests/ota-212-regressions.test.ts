...
const themeIds: Exclude<ThemeId, "custom">[] = [
  "nexus", "amoled", "oneui", "hud", "aurora", "ocean", "ember", "rose", "monochrome",
];

describe("Nexus OTA 2.1.2 regressions", () => {
  it("keeps the installed runtime while exposing the OTA release label", () => {
    const app = JSON.parse(readFileSync("app.json", "utf8")) as { expo?: { version?: string } };
    const pkg = JSON.parse(readFileSync("package.json", "utf8")) as { version?: string };
    expect(app.expo?.version).toBe("2.1.1");
    expect(pkg.version).toBe("2.1.1");
    expect(OTA_RELEASE.label).toBe("2.1.2");
  ...
  it("does not replace the themed tab bar with the white default in Focus OS", () => {
    ...
    expect(focus).toContain('tabBarStyle: active ? { display: "none" } : undefined');
    expect(focus).not.toContain('tabBarStyle: active ? { display: "none" } : {}');
  });
