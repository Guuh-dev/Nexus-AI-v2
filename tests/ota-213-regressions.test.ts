...
describe("Nexus OTA 2.1.3 Clarity", () => {
  it("moves the native runtime to 2.4.0 for systemic Mission OS recovery", () => {
    ...
    expect(JSON.parse(readFileSync("app.json", "utf8")).expo.version).toBe("2.4.0");
  });
  ...
  it("classifies this release as native because the Android widget changed", () => {
    ...
    expect(readFileSync("app.json", "utf8")).toContain('"version": "2.4.0"');
  });
});
