...
describe("Nexus OTA 2.1.3 Clarity", () => {
  it("moves the native runtime to 2.3.1 for widget recovery", () => {
    expect(OTA_RELEASE.label).toBe("2.3.1");
    expect(OTA_RELEASE.runtime).toBe("2.3.1");
    expect(JSON.parse(readFileSync("package.json", "utf8")).version).toBe("2.3.1");
    expect(JSON.parse(readFileSync("app.json", "utf8")).expo.version).toBe("2.3.1");
  });
  ...
  it("classifies this release as native because the Android widget changed", () => {
    ...
    expect(readFileSync("app.json", "utf8")).toContain('"version": "2.3.1"');
  });
});
