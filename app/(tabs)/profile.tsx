...
const themes: { id: ThemeId; label: string; description: string; color: string }[] = [
  ...
  { id: "aurora", label: "Aurora", description: "Ciano e energia sutil", color: "#22D3EE" },
  { id: "ocean", label: "Deep Ocean", description: "Azul profundo e calmo", color: "#38BDF8" },
  { id: "ember", label: "Ember", description: "Laranja de operação", color: "#F97316" },
  { id: "rose", label: "Rose Protocol", description: "Rosa futurista controlado", color: "#EC4899" },
  { id: "monochrome", label: "Monochrome", description: "Preto, branco e silêncio", color: "#E4E4E7" },
...
const areaMeta: Record<ProfileArea, { label: string; description: string }> = {
  ...
  perfil: { label: "Perfil", description: "Missão, rotina e preferências pessoais" },
...
export default function ProfileScreen() {
  ...
  const intelligenceOnline = Boolean(status?.configured && status.assistantAvailable !== false);

  const refreshStatus = () => {
    const controller = new AbortController();
    setStatusLoading(true);
    void getIntelligenceStatus(controller.signal)
      .then(setStatus)
  ...
  return (
    <>
      <Screen>
        ...
        <View style={[styles.areaNav, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          {(Object.keys(areaMeta) as ProfileArea[]).map((item) => (
            <ChoiceChip key={item} label={areaMeta[item].label} selected={area === item} onPress={() => setArea(item)} />
          ))}
        </View>
        <NexusText variant="caption" secondary>{areaMeta[area].description}</NexusText>

        {area === "central" ? (
          <>
            <Section title="Centrais do Nexus" subtitle="Cada sistema tem sua própria área para não virar um painel embolado.">
              <View style={styles.controlGrid}>
                <ControlCard title="Command Center" description="Layout, ordem e densidade" icon="▦" onPress={() => router.push("/customize")} />
                <ControlCard title="Widget Studio" description="Tamanhos, conteúdo e visual" icon="◇" onPress={() => router.push("/widget-studio")} />
                <ControlCard title="Operações" description="Projetos com fases e prazo" icon="★" onPress={() => router.push("/operations")} />
                <ControlCard title="Hábitos" description="Rotinas com recuperação" icon="♨" onPress={() => router.push("/habits")} />
                <ControlCard title="Semana" description="Capacidade em sete dias" icon="▥" onPress={() => router.push("/week")} />
                <ControlCard title="Diagnóstico" description="Rever evolução e Professor" icon="✦" onPress={() => router.push("/discovery")} />
              </View>
            </Section>

            <Section title="Estado do sistema" subtitle="Um resumo rápido antes de abrir configurações avançadas.">
              <Card style={styles.statusCard}>
                <View style={[styles.statusDot, { backgroundColor: intelligenceOnline ? colors.success : status ? colors.warning : colors.textSecondary }]} />
                <View style={styles.flex}>
                  <NexusText variant="subtitle">
                    {statusLoading ? "Verificando inteligência..." : intelligenceOnline ? "Nexus Brain online" : status?.configured ? "Backend antigo detectado" : status ? "Modo local ativo" : "Backend não alcançado"}
                  </NexusText>
              ...
              <View style={styles.summaryGrid}>
                <SummaryMetric label="Chats" value={String(data.brain.threads.length)} />
                <SummaryMetric label="Roadmaps" value={String(data.learning.roadmaps.length)} />
                <SummaryMetric label="Operações" value={String(data.operations.filter((item) => item.status === "active").length)} />
                <SummaryMetric label="Hábitos" value={String(data.habits.length)} />
              </View>
            </Section>
          </>
        ) : null}
        ...
        {area === "perfil" ? (
          <Section title="Seu comando" subtitle="Essas respostas guiam planejamento, Brain e Professor.">
                ...
                {profileDays.map((day) => {
                  ...
                  return (
                    <ChoiceChip
                      ...
                      label={day.label}
                      selected={selected}
                      onPress={() => nextDays.length > 0 && updateProfile({ activeDays: nextDays })}
                    />
                  );
                })}
              </View>
        ...
        {area === "visual" ? (
          <>
            <Section title="Temas" subtitle="Mude o clima inteiro sem alterar seus dados.">
                ...
                {themes.map((theme) => (
                  <Pressable
                    ...
                    style={[
                      styles.themeCard,
                      {
                        backgroundColor: data.preferences.theme === theme.id ? `${theme.color}18` : colors.surface,
                        borderColor: data.preferences.theme === theme.id ? theme.color : colors.border,
                      },
                    ]}
                  >
              ...
              {data.preferences.theme === "custom" ? (
                <Card style={styles.customColorCard}>
                  ...
                  <NexusButton label="Aplicar cor" variant="secondary" onPress={applyCustomAccent} fullWidth />
                </Card>
              ) : null}
            </Section>

            <Section title="Conforto" subtitle="Ajustes de movimento, vibração e som.">
              ...
              <NexusButton label="Abrir Command Center" variant="secondary" onPress={() => router.push("/customize")} fullWidth />
              <NexusButton label="Abrir Widget Studio" variant="ghost" onPress={() => router.push("/widget-studio")} fullWidth />
            </Section>
          </>
        ) : null}
        ...
        {area === "sistema" ? (
          <>
            <Section title="Inteligência" subtitle="A chave da OpenRouter continua exclusivamente no backend.">
              <Card style={styles.statusCard}>
                <View style={[styles.statusDot, { backgroundColor: intelligenceOnline ? colors.success : status ? colors.warning : colors.danger }]} />
                <View style={styles.flex}>
                  <NexusText variant="subtitle">{intelligenceOnline ? "OpenRouter configurado" : status?.configured ? "Backend precisa da V2.1" : status ? "Fallback local" : "Backend indisponível"}</NexusText>
                  <NexusText variant="caption" secondary>
                    {intelligenceOnline
                      ? `API ${status?.apiVersion ?? "sem versão"} • Assistente disponível`
                      : "A V2.1 tenta detectar automaticamente backend antigo ou fora do ar."}
                  </NexusText>
                </View>
              </Card>
              ...
              <Card style={[styles.infoCard, { backgroundColor: `${colors.primary}0C` }]}>
                ...
                <NexusText variant="caption" secondary>
                  O app mantém captura, revisão e planejamento local. Para reativar Brain e Atlas online, publique esta versão do backend e configure OPENROUTER_API_KEY no Render.
                </NexusText>
              </Card>
            </Section>
        ...
        {area === "dados" ? (
          <>
            <Section title="Backup e privacidade" subtitle="Seus dados ficam locais até você exportá-los.">
              ...
              <Card style={styles.privacyCard}>
                ...
                <NexusText variant="caption" secondary>
                  Perfil, tarefas, XP, chats e roadmaps ficam no armazenamento privado do app. Somente contexto sanitizado e limitado é enviado ao backend.
                </NexusText>
              </Card>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { resetToday(); setResetTodayOpen(false); }}
      />
      <ConfirmDialog
        ...
        onConfirm={() => { setResetAllOpen(false); void resetAll().then(() => router.replace("/onboarding")); }}
      />
    </>
  );
}
...
function ControlCard({ title, description, icon, onPress }: { title: string; description: string; icon: string; onPress: () => void }) {
  const { colors } = useNexus();
  return (
    <Pressable onPress={onPress} style={[styles.controlCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
      <View style={[styles.controlIcon, { backgroundColor: `${colors.primary}18` }]}><NexusText variant="title" color={colors.primarySoft}>{icon}</NexusText></View>
  ...
  return (
    <View style={[styles.switchRow, { borderBottomColor: colors.border }]}>
      <View style={styles.flex}>
        ...
        {description ? <NexusText variant="caption" secondary>{description}</NexusText> : null}
      </View>
...
const styles = StyleSheet.create({
  ...
  areaNav: { flexDirection: "row", flexWrap: "wrap", gap: 7, borderWidth: 1, borderRadius: 18, padding: 8, marginTop: 14, marginBottom: 8 },
  section: { marginTop: 27, gap: 13 },
  sectionTitle: { gap: 4 },
  choiceGroup: { gap: 9 },
  controlGrid: { flexDirection: "row", flexWrap: "wrap", gap: 10 },
  controlCard: { width: "47%", flexGrow: 1, minHeight: 132, borderRadius: 19, borderWidth: 1, padding: 14, gap: 7 },
  controlIcon: { width: 38, height: 38, borderRadius: 13, alignItems: "center", justifyContent: "center" },
  summaryGrid: { flexDirection: "row", flexWrap: "wrap", gap: 8 },
  summaryMetric: { width: "47%", flexGrow: 1, minHeight: 82, justifyContent: "center", gap: 4 },
  themeGrid: { flexDirection: "row", flexWrap: "wrap", gap: 9 },
  themeCard: { width: "47%", flexGrow: 1, minHeight: 118, borderRadius: 18, borderWidth: 1, padding: 14, gap: 7 },
