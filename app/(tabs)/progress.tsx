...
export default function ProgressScreen() {
  ...
  const completedMissions = data.history.filter((day) => day.plan.mainMission.completed).slice(-5).reverse();
    ...
    <Screen>
      <View style={styles.header}>
        <View style={styles.flex}>
          ...
          <NexusText secondary>Número bonito sem execução é decoração. Aqui entra o que foi concluído.</NexusText>
        </View>
      ...
      <View style={styles.streakRow}>
        <Card style={styles.streakCard}>
          <NexusText color={colors.warning} variant="display">♨ {data.progress.currentStreak}</NexusText>
          <NexusText variant="caption" secondary>Sequência atual</NexusText>
        </Card>
        <Card style={styles.streakCard}>
          <NexusText variant="display">{data.progress.bestStreak}</NexusText>
          <NexusText variant="caption" secondary>Melhor sequência</NexusText>
        </Card>
      </View>

      <View style={styles.section}>
        <View style={styles.sectionHeader}><View style={styles.flex}><NexusText variant="title">Revisão semanal com IA</NexusText><NexusText variant="caption" secondary>Padrões, consistência e próximo desafio.</NexusText></View><PixelMascot state={assistantBusy ? "thinking" : "idle"} size={44} /></View>
        ...
        {latestReview ? <Card style={styles.review}><View style={styles.categoryTop}><NexusText variant="mono" color={latestReview.source === "ai" ? colors.success : colors.warning}>{latestReview.source === "ai" ? "ANÁLISE NEXUS" : "ANÁLISE LOCAL"}</NexusText><NexusText variant="title">{latestReview.consistencyScore}/100</NexusText></View><NexusText variant="title">Foco da próxima semana</NexusText><NexusText>{latestReview.nextWeekFocus}</NexusText><ProgressBar progress={latestReview.consistencyScore / 100} color={latestReview.consistencyScore >= 70 ? colors.success : colors.warning} /><ReviewList title="O que funcionou" items={latestReview.keep} /><ReviewList title="Padrões observados" items={latestReview.patterns} /><ReviewList title="O que cortar" items={latestReview.cut} /><Card style={[styles.challenge, { backgroundColor: `${colors.primary}10` }]}><NexusText variant="mono" color={colors.primarySoft}>DESAFIO DA SEMANA</NexusText><NexusText variant="subtitle">{latestReview.challenge}</NexusText></Card></Card> : null}
      </View>

      <View style={styles.section}>
        <NexusText variant="title">Atributos do executor</NexusText>
        <Card style={styles.categoryCard}>{Object.entries(data.progress.attributes).map(([name, value]) => <View key={name} style={styles.categoryRow}><View style={styles.categoryTop}><NexusText variant="subtitle">{({ foco: "Foco", execucao: "Execução", consistencia: "Consistência", disciplina: "Disciplina" } as Record<string,string>)[name]}</NexusText><NexusText variant="mono" color={colors.primarySoft}>{value}</NexusText></View><ProgressBar progress={value / maxAttribute} height={7} /></View>)}</Card>
      </View>

      {data.progress.challenges.length ? <View style={styles.section}><NexusText variant="title">Desafios ativos</NexusText><View style={styles.achievementGrid}>{data.progress.challenges.filter((challenge) => new Date(challenge.expiresAt).getTime() >= Date.now() - 86_400_000).map((challenge) => <Card key={challenge.id} style={[styles.achievement, { borderColor: challenge.completed ? `${colors.success}55` : challenge.type === "boss" ? `${colors.danger}55` : colors.border }]}><NexusText variant="mono" color={challenge.type === "boss" ? colors.danger : colors.primarySoft}>{challenge.type === "boss" ? "BOSS" : challenge.type.toUpperCase()}</NexusText><NexusText variant="subtitle">{challenge.title}</NexusText><NexusText variant="caption" secondary>{challenge.description}</NexusText><ProgressBar progress={Math.min(1, challenge.progress / challenge.target)} color={challenge.completed ? colors.success : colors.primary} /><NexusText variant="caption" color={challenge.completed ? colors.success : colors.textSecondary}>{challenge.completed ? `✓ +${challenge.xpReward} XP` : `${challenge.progress}/${challenge.target}`}</NexusText></Card>)}</View></View> : null}
